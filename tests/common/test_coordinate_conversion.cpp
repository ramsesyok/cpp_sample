...
TEST_CASE("GeoToEcef: 赤道・本初子午線・標高0", "[coord_conv][ecef]") {
  ...
  const EcefPosition p = GeoToEcef(g);
...
TEST_CASE("GeoToEcef: 標高が直接 X に加算される (赤道)",
          "[coord_conv][ecef]") {
...
TEST_CASE("GeoToEcef: 原点からの距離が a に近い (赤道)",
          "[coord_conv][ecef]") {
...
TEST_CASE("変換ラウンドトリップ (deg <-> rad)", "[coord_conv]") {
  ...
  const MapCoordinate round = ToMap(ToGeo(original));
  REQUIRE(round.latitude_deg() ==
          Catch::Approx(original.latitude_deg()));
  REQUIRE(round.longitude_deg() ==
          Catch::Approx(original.longitude_deg()));
  REQUIRE(round.altitude_m() == Catch::Approx(original.altitude_m()));
}

TEST_CASE("EcefToGeo: 赤道・本初子午線", "[coord_conv][ecef]") {
  // (a, 0, 0) -> (φ=0, λ=0, h=0)
  const GeoCoordinate g = EcefToGeo(EcefPosition{6378137.0, 0.0, 0.0});
...
TEST_CASE("EcefToGeo: 赤道・東経90度", "[coord_conv][ecef]") {
  // (0, a, 0) -> (φ=0, λ=π/2, h=0)
  const GeoCoordinate g = EcefToGeo(EcefPosition{0.0, 6378137.0, 0.0});
  REQUIRE(g.latitude() == Catch::Approx(0.0).margin(1e-12));
  REQUIRE(g.longitude() == Catch::Approx(1.5707963267948966));
  REQUIRE(g.altitude() == Catch::Approx(0.0).margin(1e-6));
}
...
TEST_CASE("EcefToGeo: 北極 (極近傍ガード経路)", "[coord_conv][ecef]") {
  ...
  const GeoCoordinate g =
      EcefToGeo(EcefPosition{0.0, 0.0, 6356752.314245179});
...
TEST_CASE("EcefToGeo: 南極上空 1000m", "[coord_conv][ecef]") {
  ...
  const GeoCoordinate g =
      EcefToGeo(EcefPosition{0.0, 0.0, -(6356752.314245179 + 1000.0)});
  REQUIRE(g.latitude() == Catch::Approx(-1.5707963267948966));
  REQUIRE(g.altitude() == Catch::Approx(1000.0).margin(1e-6));
}

TEST_CASE("ecef <-> geo ラウンドトリップ (各種地点)",
          "[coord_conv][ecef]") {
  ...
  for (const auto& c : cases) {
    const GeoCoordinate original =
        ToGeo(MapCoordinate{c.lat_deg, c.lon_deg, c.alt_m});
    const GeoCoordinate round = EcefToGeo(GeoToEcef(original));
    INFO("case lat_deg=" << c.lat_deg << " lon_deg=" << c.lon_deg
                         << " alt_m=" << c.alt_m);
    REQUIRE(round.latitude() ==
            Catch::Approx(original.latitude()).margin(1e-10));
    REQUIRE(round.longitude() ==
            Catch::Approx(original.longitude()).margin(1e-10));
    REQUIRE(round.altitude() ==
            Catch::Approx(original.altitude()).margin(1e-6));
  }
}
