#ifndef MYAPP_COMMON_ECEF_COORDINATE_H
#define MYAPP_COMMON_ECEF_COORDINATE_H
...
class EcefCoordinate {
 public:
  ...
  EcefCoordinate operator+(const Vector3D& displacement) const {
    return EcefCoordinate{v_.x_ + displacement.x_, v_.y_ + displacement.y_,
  ...
  EcefCoordinate operator*(double s) const {
    return EcefCoordinate{v_.x_ * s, v_.y_ * s, v_.z_ * s};
  }
...
#endif  // MYAPP_COMMON_ECEF_COORDINATE_H
